...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\程式\\psychopy\\time_attention\\at_1\\timeperception_at\\time_exp_at_lastrun.py',
...
_start_3_allKeys = []
# keep track of which components have finished
startComponents = [start_2, start_3]
...
while continueRoutine:
    ...
    if start_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'start_3.started')
        start_3.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(start_3.clock.reset)  # t=0 on next screen flip
    ...
    if start_3.status == STARTED and not waitOnFlip:
        theseKeys = start_3.getKeys(keyList=['space'], waitRelease=False)
...
for thisComponent in startComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if start_3.keys in ['', [], None]:  # No response was made
    start_3.keys = None
thisExp.addData('start_3.keys',start_3.keys)
if start_3.keys != None:  # we had a response
    thisExp.addData('start_3.rt', start_3.rt)
thisExp.nextEntry()
...
trials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('pic_condition_2.xlsx'),
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        # *redstar* updates
        if redstar.status == NOT_STARTED and tThisFlip >= starduration-frameTolerance:
            # keep track of start time/frame for later
            redstar.frameNStart = frameN  # exact frame index
            redstar.tStart = t  # local t and not account for scr refresh
            redstar.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(redstar, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'redstar.started')
            redstar.setAutoDraw(True)
        if redstar.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > redstar.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
                redstar.tStop = t  # not accounting for scr refresh
                redstar.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'redstar.stopped')
                redstar.setAutoDraw(False)
